...
def main(args):
    ...
    log_string("The number of training batches is: %s" % str(len(TRAIN_DATASET)/BATCH_SIZE))
    log_string("The number of test batches is: %s" % str(len(TEST_DATASET)/BATCH_SIZE))
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9): # target = label
            ...
            # points[:, :, :3] = provider.rotate_point_cloud_z(points[:, :, :3]) # not sure how but performs rotation to "align point cloud"
        ...
        train_mean_loss.append((loss_sum / num_batches).item())
        train_mean_accuracy.append(total_correct / float(total_seen))
        ...
        with torch.no_grad():
            ...
            print(f'Average evaluation time: {total_time/len(testDataLoader)}')   
            times += total_time/len(testDataLoader)
            ...
            for l in range(NUM_CLASSES):
                iou_per_class_str += 'class %s weight: %.3f, IoU: %.3f \n' % (
                    seg_label_to_cat[l] + ' ' * (7 - len(seg_label_to_cat[l])), labelweights[l - 1],
            ...
            test_mean_loss.append((loss_sum / num_batches).item())
            test_mean_accuray.append(total_correct / float(total_seen))

            if mIoU >= best_iou or not os.path.exists('log/sem_seg/pointnet2_sem_seg/checkpoints/best_model.pth'):
                best_iou = mIoU
                logger.info('Save model...')
                ...
                torch.save(state, savepath)
                torch.save(state, 'log/sem_seg/pointnet2_sem_seg/checkpoints/best_model.pth')
                log_string('Saving model....')
            log_string('Best mIoU: %f' % best_iou)

        global_epoch += 1

    print(f'Average Time for Eval for 1 Episode: {times/args.epoch}')
